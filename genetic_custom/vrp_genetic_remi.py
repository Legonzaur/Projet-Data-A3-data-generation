...
dates = list(\
             vehicles_stamped.distinct("date")) #SOLUTION
# Initialisation de vehicules_par_minutes
vehicules_par_minutes = dict(\
                             [(date.strftime("%H:%M"), 0) for date in dates]) #SOLUTION

# Parcours de la collection et calcul de la somme du trafic
for trafic in vehicles_stamped.find():
    date = trafic["date"].strftime("%H:%M") #SOLUTION
    vehicules_par_minutes[date] += trafic["nb_vehicules"] #SOLUTION

nb_jours = 5
nb_data_points = nb_jours*len(vehicles_stamped.distinct("num_arete"))
trafics = \
    [vehicules_par_minutes[date]/nb_data_points for date in vehicules_par_minutes.keys()]#SOLUTION
...
# Les dates du soir
xs = \
    pd.date_range("2020-01-01 17:01", "2020-01-01 19:00", freq = "min").to_pydatetime().tolist() #SOLUTION# Les données de trafic au soir
ys = \
    trafics[120:] #SOLUTION
# Traitements sur l'affichage au soir
axes[1].xaxis.set_major_locator(mdates.HourLocator(interval = 1)) #SOLUTION
axes[1].xaxis.set_minor_locator(mdates.MinuteLocator(interval = 15)) #SOLUTION
axes[1].xaxis.set_major_formatter(mdates.DateFormatter('%H:%M')) #SOLUTION
axes[1].set(xlabel="temps", ylabel="trafic") #SOLUTION
axes[1].plot(xs,ys,"o") #SOLUTION
