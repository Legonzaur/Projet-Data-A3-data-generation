...
congestion_table = [0.05, 0.1, 0.1, 0.05, 0.05, 0.2, 0.6, 0.9, 1, 0.7, 0.5, 0.7, 0.9, 0.9, 0.7, 0.5, 0.7, 0.9, 1, 0.7, 0.5, 0.4, 0.3, 0.1]

temp_matrix = copy.deepcopy(initial_matrix)


def generate_traffic(graph, source, target):
    traffic_table = [round(congestion * random.randint(10, 100) * ((gg.count_neighbors(graph, source) + gg.count_neighbors(graph, target)) / 2), 2) for congestion in congestion_table]
...
def browse_matrix(matrix):
    ...
    for ligneIndex, ligne in enumerate(matrix):
        for colonneIndex, case in enumerate(ligne):
            ...
            if case == 1:
                output[ligneIndex][colonneIndex] = generate_traffic(matrix, ligneIndex, colonneIndex)
